...
def femurShaft_ns(dataModel, femur_start, answerLeg, rightbone, femurMuscle_start, markerFemur_start, wrapLocations):
    ...
    wrapLocationsRot = np.dot(R_transfer, wrapLocations).T
    ...
    for i in range(markerFemur_start.shape[0]):
        item = np.dot(R_transfer, markerFemur_start[i, :])
    ...
    print('ShaftProxMA')
    print(ShaftProxMA)
    middleBoxMarker = np.concatenate((LesserTrocMarkers, ShaftProxMA), axis=0)
